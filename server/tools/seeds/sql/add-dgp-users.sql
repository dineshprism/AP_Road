-- Add DGP/ADGP users to the correct table (likely public.users)
...
INSERT INTO profiles (id, user_id, full_name, district, designation) VALUES 
...
SELECT email, 'public.users' as table_name FROM users WHERE email IN ('dgp', 'adgp')
UNION ALL
SELECT email, 'auth.users' as table_name FROM auth.users WHERE email IN ('dgp', 'adgp');
