...
const GoogleAccidentMap = ({
  ...
  useEffect(() => {
    ...
    if (boundariesEnabled) {
      dataLayer.addGeoJson(districtGeoJson as any);
  ...
  if (!apiKey || loadError) {
    return <LeafletFallbackMap accidents={accidents} userDistrict={userDistrict} height={height} showHeatmap={showHeatmap} showDistrictBoundaries={showDistrictBoundaries} zoom={zoom} />;
  }

  if (!isLoaded) {
    return (
      <div
        className="flex items-center justify-center rounded-[24px] border border-slate-200 bg-slate-50 text-sm font-medium text-slate-600"
        style={{ height }}
      >
        Loading Google Maps...
      </div>
    ...
    <div className="relative overflow-hidden rounded-[24px]" style={{ height }}>
          ...
          >
            <div style={{ minWidth: 220 }}>
              ...
              <h4 style={{ margin: "4px 0 0", fontSize: 14, fontWeight: 700, color: "#0f172a" }}>
                {selectedPoint.place_of_accident}
              </h4>
      ...
      <div className="absolute bottom-3 right-3 z-[1000] flex w-[176px] flex-col gap-2 rounded-2xl border border-slate-200/90 bg-white/92 p-2 shadow-[0_18px_50px_-24px_rgba(15,23,42,0.42)] backdrop-blur md:bottom-auto md:right-4 md:top-4 md:w-[220px]">
        <div className="grid grid-cols-3 gap-1 rounded-xl bg-slate-100 p-1">
          <button
            className={`rounded-lg px-2 py-2 text-xs font-semibold transition ${mapTypeId === "roadmap" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:bg-white/80"}`}
            onClick={() => setMapTypeId("roadmap")}
          >
            Street
          </button>
          <button
            className={`rounded-lg px-2 py-2 text-xs font-semibold transition ${mapTypeId === "satellite" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:bg-white/80"}`}
            onClick={() => setMapTypeId("satellite")}
          >
            Sat
          </button>
          <button
            className={`rounded-lg px-2 py-2 text-xs font-semibold transition ${mapTypeId === "terrain" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:bg-white/80"}`}
        ...
        <button
          ...
          onClick={() => fitMapToScope(mapRef.current, districtBounds, districtLocked ? 10 : zoom)}
        >
          Recenter Map
        </button>
