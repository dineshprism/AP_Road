          ...
          <TabsContent value="causes" className="space-y-5">
            <div className="grid gap-5 xl:grid-cols-2">
              <Card className="shadow-sm">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={360}>
                    <BarChart data={driverCauseChartData} layout="vertical" margin={{ left: 16, right: 16 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                      <XAxis type="number" stroke="#64748b" />
                      <YAxis dataKey="shortCause" type="category" width={130} stroke="#64748b" tick={{ fontSize: 11 }} />
                      <Tooltip formatter={(value: number, _name, props: any) => [value, props?.payload?.cause || props?.payload?.shortCause]} />
                      <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                        {driverCauseChartData.map((entry) => (
                          <Cell key={entry.cause} fill={entry.severity === "high" ? "#aa3d47" : entry.severity === "medium" ? "#c75b12" : "#d39d11"} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                  <div className="mt-4 space-y-2">
                    {driverCauseChartData.slice(0, 5).map((entry, index) => (
                      <div key={entry.cause} className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
